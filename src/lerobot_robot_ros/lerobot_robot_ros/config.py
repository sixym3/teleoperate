...
class ROS2InterfaceConfig:
    ...
    # Topic names for ROS2 communication
    arm_controller_name: str = "arm_controller"  # Used for trajectory control
    position_controller_name: str = "position_controller"  # Used for position control
    velocity_controller_name: str = "velocity_controller"  # Used for velocity control
    gripper_controller_name: str = "gripper_controller"

    # Only applicable if cartesian velocity control is used.
    max_linear_velocity: float = 0.10
    max_angular_velocity: float = 0.25  # rad/s

    # Only applicable if joint velocity control is used.
    max_joint_velocities: list[float] | None = None  # rad/s for revolute joints
...
@RobotConfig.register_subclass("ur5e_ros")
@dataclass
class UR5eConfig(ROS2Config):
    """Configuration for Universal Robots UR5e with ROS2 control.

    This configuration uses the Universal_Robots_ROS2_Driver package
    (https://github.com/UniversalRobots/Universal_Robots_ROS2_Driver).

    Compatible with:
    - Universal Robots UR5e arm
    - Standard UR joint trajectory controller (scaled_joint_trajectory_controller)
    - Common end-effectors (e.g., Robotiq 2F-85 gripper)
    """
    ...
    ros2_interface: ROS2InterfaceConfig = field(
        default_factory=lambda: ROS2InterfaceConfig(
            ...
            base_link="base_link",
            # Universal Robots driver uses "scaled_joint_trajectory_controller"
            arm_controller_name="scaled_joint_trajectory_controller",
            gripper_controller_name="robotiq_gripper_controller",
            # UR5e joint limits (radians) - slightly conservative for safety
            min_joint_positions=[-6.28, -6.28, -3.14, -6.28, -6.28, -6.28],
            max_joint_positions=[6.28, 6.28, 3.14, 6.28, 6.28, 6.28],
            # Robotiq 2F-85 gripper positions (adjust based on your gripper)
...
@RobotConfig.register_subclass("ur5e_ros_velocity")
@dataclass
class UR5eVelocityConfig(ROS2Config):
    """Configuration for Universal Robots UR5e with ROS2 joint velocity control.

    This configuration uses the built-in forward_velocity_controller from the
    Universal Robots ROS2 driver. It's useful for teleoperation and real-time control.

    Compatible with:
    - Universal Robots UR5e arm
    - forward_velocity_controller (built-in UR driver controller)
    - Common end-effectors (e.g., Robotiq 2F-85 gripper)

    The forward_velocity_controller is a built-in velocity controller in the
    Universal_Robots_ROS2_Driver that directly sends velocity commands to the robot.
    """
    ...
    ros2_interface: ROS2InterfaceConfig = field(
        default_factory=lambda: ROS2InterfaceConfig(
            ...
            gripper_joint_name=None,  # Set to gripper joint name if using gripper
            base_link="base_link",
            # Use UR driver's built-in velocity controller
            velocity_controller_name="forward_velocity_controller",
            gripper_controller_name="robotiq_gripper_controller",
            # UR5e joint velocity limits (rad/s) - conservative for safety
            # These values are normalized, so input will be in range [-1, 1]
            max_joint_velocities=[1.0, 1.0, 1.5, 1.5, 1.5, 1.5],
            # Robotiq 2F-85 gripper positions (adjust based on your gripper)
            gripper_open_position=0.0,
            gripper_close_position=0.8,
            gripper_action_type=GripperActionType.TRAJECTORY,
