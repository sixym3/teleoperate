...
class ROS2Interface:
    ...
    This class supports multiple control modes from ros2_control for arm control:

    - ActionType.JOINT_POSITION:
      Uses JointGroupPositionController.
      Publishes Float64MultiArray messages to '/{position_controller_name}/commands'

    - ActionType.JOINT_TRAJECTORY:
      Uses JointTrajectoryController.
      Publishes JointTrajectory messages to '/{arm_controller_name}/joint_trajectory'

    - ActionType.JOINT_VELOCITY:
      Uses JointGroupVelocityController.
      Publishes Float64MultiArray messages to '/{velocity_controller_name}/commands'

    - ActionType.CARTESIAN_VELOCITY:
      Uses MoveIt Servo for end-effector velocity control.
    ...
    Controller names are configurable via ROS2InterfaceConfig (arm_controller_name,
    position_controller_name, velocity_controller_name, gripper_controller_name)
    to support different robot drivers.
    """
    ...
    def __init__(self, config: ROS2InterfaceConfig, action_type: ActionType):
        ...
        self.vel_cmd_pub: Publisher | None = None
        ...
        self.moveit2_servo: object | None = None  # MoveIt2Servo when using CARTESIAN_VELOCITY
    ...
    def connect(self) -> None:
        ...
        if self.action_type == ActionType.JOINT_POSITION:
        ...
        elif self.action_type == ActionType.JOINT_VELOCITY:
            self.vel_cmd_pub = self.robot_node.create_publisher(
                Float64MultiArray, f"/{self.config.velocity_controller_name}/commands", 10
            )
            ...
            # Import MoveIt Servo only when needed (requires MoveIt2 with Servo support)
    ...
    def send_joint_velocity_command(self, joint_velocities: list[float], normalize: bool = True) -> None:
        """
        Send a velocity command to the robot's joints.
        Args:
            joint_velocities (list[float]): The target velocities for the joints.
            normalize (bool): Whether to scale velocities based on max_joint_velocities.
                             If True, input velocities are in range [-1, 1].
        """
        if not self.robot_node:
            raise DeviceNotConnectedError("ROS2Interface is not connected. You need to call `connect()`.")

        if len(joint_velocities) != len(self.config.arm_joint_names):
            raise ValueError(
                f"Expected {len(self.config.arm_joint_names)} joint velocities, but got {len(joint_velocities)}."
            )

        if normalize:
            if self.config.max_joint_velocities is None:
                raise ValueError(
                    "Joint velocity normalization requires max_joint_velocities to be set."
                )
            joint_velocities = [
                vel * max_vel
                for vel, max_vel in zip(
                    joint_velocities,
        ...
        if self.vel_cmd_pub is None:
            raise DeviceNotConnectedError("Velocity command publisher is not initialized.")
    ...
    def _joint_state_callback(self, msg: "JointState") -> None:
        ...
        if self.config.gripper_joint_name:
            ...
            if idx is not None:
                # Only add gripper if it exists in joint states
                positions[self.config.gripper_joint_name] = msg.position[idx]
                velocities[self.config.gripper_joint_name] = msg.velocity[idx]
            else:
                logger.debug(f"Gripper joint '{self.config.gripper_joint_name}' not found in joint state (skipping)")
    ...
    def disconnect(self):
        ...
        if self.vel_cmd_pub:
            self.vel_cmd_pub.destroy()
            self.vel_cmd_pub = None
