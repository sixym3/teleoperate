...
class ROS2Robot(Robot):
    ...
    def action_features(self) -> dict[str, type]:
        ...
        elif self.config.action_type == ActionType.JOINT_VELOCITY:
            return {f"{joint}.vel": float for joint in self.config.ros2_interface.arm_joint_names} | {
                "gripper.pos": float
            }
        else:
    ...
    def send_action(self, action: dict[str, float]) -> dict[str, float]:
        ...
        if self.config.action_type == ActionType.CARTESIAN_VELOCITY:
            ...
            if self.config.max_relative_target is not None:
                # We don't have the current velocity of the arm, so set it to 0.0
                # Effectively the goal velocity gets clipped by max_relative_target
                goal_present_vel = {key: (act, 0.0) for key, act in action.items() if key != "gripper.pos"}
                action = ensure_safe_goal_position(goal_present_vel, self.config.max_relative_target)

            joint_velocities = [action[joint + ".vel"] for joint in self.config.ros2_interface.arm_joint_names]
            self.ros2_interface.send_joint_velocity_command(joint_velocities)
        elif self.config.action_type in (ActionType.JOINT_POSITION, ActionType.JOINT_TRAJECTORY):
            if self.config.max_relative_target is not None:
                goal_present_pos = {}
                joint_state = self.ros2_interface.joint_state
        ...
        # Only send gripper command if present in action
...
class UR5eROS(ROS2Robot):
    pass


class UR5eROSVelocity(ROS2Robot):
    pass
